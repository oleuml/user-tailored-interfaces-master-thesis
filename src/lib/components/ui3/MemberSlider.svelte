<script lang="ts">
  ...
  if (proposedScore < 0 || proposedScore > 1) {
    throw new Error(`proposedScore must be betweennumber or equals 0.0 and 1.0: ${proposedScore}`);
  }
  if (threshold < 0 || threshold > 1) {
    throw new Error(`threshold must be betweennumber or equals 0.0 and 1.0: ${threshold}`);
  }
  if (score < 0 || score > 1) {
    throw new Error(`score must be betweennumber or equals 0.0 and 1.0: ${score}`);
  }
</script>

<div class="flex h-full">
  <Slider bind:score bind:threshold>
